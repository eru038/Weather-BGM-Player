  <script>
    ...
    window.__CURRENT_PAGE__ = 'home'; // 记录当前页面

    // 根据 ?from=xxx 回到原页面（授权回调后触发）
        ...
        if (page === "home") {
          content.innerHTML = `
            ...
            <div style="margin-top:16px;">
              <button class="spotify-btn" onclick="playWeatherOnSpotify()">在 Spotify 播放 天気BGM</button>
            </div>
            ...
            <div id="player-bar">
              ...
              <a id="open-in-spotify" href="#" target="_blank">在 Spotify 中打开</a>
              <span id="premium-hint" class="muted">需要 Premium 才能网页内控制</span>
            </div>
          `;
          ...
          renderUserPlaylists();   // ✅ 新增：加载并渲染歌单
          ...
          `;
          setupColorPicker(); initControls(); // 保证按钮文案更新
        }
    ...
    function bindControlEvents(){
      ...
      if (window.Spotify && spotifyPlayer) {
        spotifyPlayer.addListener('player_state_changed', state => {
          ...
          if (btnPlay) btnPlay.textContent = isPaused ? '▶' : '⏸︎';
...
// 既存の WEATHER_TO_QUERY と組み合わせ
  async function pickSpotifyAndPlay(weatherMain){
    ...
    async function ensureActiveDevice() {
      ...
      alert('找不到活跃的 Spotify 设备。请先在 Spotify 客户端/网页随便播放一首歌，然后再点播放。');
      return false;
    }
    ...
    async function renderUserPlaylists(){
      ...
      try {
        ...
        if (!items.length) {
          grid.innerHTML = `<div style="opacity:.8;">歌単が見つかりませんでした。</div>`;
          return;
        }
          ...
          return `
            <div class="pl-card" data-plid="${p.id}" data-uri="${p.uri}" data-ext="${p.external_url || ''}" style="background:rgba(0,0,0,.25);border:1px solid #222;border-radius:14px;padding:10px;">
              <div class="pl-cover" style="position:relative;cursor:pointer;">
                ${img}
                <button class="play-btn" title="播放" style="position:absolute;right:8px;bottom:8px;border:none;border-radius:18px;padding:8px 12px;background:#1DB954;color:#fff;cursor:pointer;">▶</button>
              </div>
        ...
        grid.innerHTML = `<div style="opacity:.8;">歌単の取得に失敗しました。</div>`;
      }
    }

    // Premium → 网页内直接播放；非会员 → 跳到 Spotify
    ...
    async function renderTracksForPlaylist(playlistId, cardEl, panelEl){
      ...
      try{
        ...
        panelEl.innerHTML = `
          <h3 style="margin-top:12px;">曲目（前 ${items.length} 首）</h3>
          <div style="display:grid;grid-template-columns:1fr;gap:10px;margin-top:8px;">
            ${items.map((t,i)=>`
              <div style="display:flex;gap:10px;align-items:center;background:rgba(0,0,0,.25);border:1px solid #222;border-radius:10px;padding:8px;">
                ...
                <button onclick="openTrackExternally('${t.external_url || ''}')" style="border:1px solid #333;background:transparent;color:#e6eef3;border-radius:8px;padding:6px 10px;cursor:pointer;">打开</button>
              </div>
            `).join('')}
          </div>
        `;
